...
public class ContactListOld extends AbstractList {
    ...
    public boolean add(String name) {
        ...
        if (findInsertLocation(name) == contactList.size() ||
                !name.equals(contactList.get(findInsertLocation(name)))) {
    ...
    public void add(ArrayList<String> names) {
        ...
        for (String name : names) {
            if (!contactList.get(findInsertLocation(name)).equals(name)) {
                add(name);
            }
        }
    }
    ...
    public boolean remove(String name) {
        ...
        int nameLocation = findInsertLocation(name);
        if (nameLocation < contactList.size() && name.equals(contactList.get(nameLocation))) {
            System.out.println("- Removing " + name);
            contactList.remove(nameLocation);
            return true;
        }
        return false;
    }
